...
class BP_Groups_Group {
	...
	public function populate() {
		...
		if ( $group = $wpdb->get_row( $wpdb->prepare( "SELECT g.* FROM {$bp->groups->table_name} g WHERE g.id = %d", $this->id ) ) ) {
			...
			$this->place              = stripslashes($group->place);
			$this->place_coordinates  = stripslashes($group->place_coordinates);
			$this->for_whom  		  = stripslashes($group->for_whom);
			$this->master_fio  		  = stripslashes($group->master_fio);
			$this->master_telephone   = stripslashes($group->master_telephone);
			$this->master_email  	  = stripslashes($group->master_email);
	...
	public function save() {
		...
		$this->place        = apply_filters( 'groups_group_place_before_save',        $this->place,        $this->id );
		$this->place_coordinates   = apply_filters( 'groups_group_place_coordinates_before_save',        $this->place_coordinates,        $this->id );
		$this->for_whom            = apply_filters( 'groups_group_for_whom_before_save',        $this->for_whom,        $this->id );
		$this->master_fio      	   = apply_filters( 'groups_group_master_fio_before_save',        $this->master_fio,        $this->id );
		$this->master_telephone    = apply_filters( 'groups_group_master_telephone_before_save',        $this->master_telephone,        $this->id );
		$this->master_email        = apply_filters( 'groups_group_master_email_before_save',        $this->master_email,        $this->id );
		...
		if ( !empty( $this->id ) ) {
			...
			$sql = $wpdb->prepare(
				"INSERT INTO {$bp->groups->table_name} (
					...
					$this->name,
					$_POST['group-place'],
					$_POST['group-place-coordinates'],
					$_POST['group-for-whom'],
					$_POST['group-master-fio'],
					$_POST['group-master-telephone'],
					$_POST['group-master-email'],
					$this->slug,
					$this->description,
					$this->status,
					$this->enable_forum,
