...
function groups_create_group( $args = '' ) {
	...
	if ( empty( $group_id ) ) {
		...
		$member->save();
                ...
                $date_start = groups_get_groupmeta( $group->id, 'group_plus_header_fieldone');

		groups_update_groupmeta( $group->id, 'last_activity', $date_start );
...
function groups_edit_base_group_details( $group_id, $group_name, $group_desc, $notify_members ) {
	...
	$group->place = $_POST['group-place'];
	$group->place_coordinates = $_POST['group-place-coordinates'];
	$group->for_whom = $_POST['group-for-whom'];
	$group->master_fio = $_POST['group-master-fio'];
	$group->master_telephone = $_POST['group-master-telephone'];
	$group->master_email = $_POST['group-master-email'];
...
function groups_join_group( $group_id, $user_id = 0 ) {
	...
	// Modify group meta
	$date_start = groups_get_groupmeta( $group_id, 'group_plus_header_fieldone');

        groups_update_groupmeta( $group_id, 'last_activity', $date_start );
