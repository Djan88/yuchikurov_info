...
function bp_group_place( $group = false ) {
	echo bp_get_group_place( $group );
}
	function bp_get_group_place( $group = false ) {
		global $groups_template;

		if ( empty( $group ) )
			$group =& $groups_template->group;

		return apply_filters( 'bp_get_group_place', $group->place );
	}
function bp_group_place_coordinates( $group = false ) {
	echo bp_get_group_place_coordinates( $group );
}
	function bp_get_group_place_coordinates( $group = false ) {
		global $groups_template;

		if ( empty( $group ) )
			$group =& $groups_template->group;

		return apply_filters( 'bp_get_group_place_coordinates', $group->place_coordinates );
	}
function bp_group_for_whom( $group = false ) {
	echo bp_get_group_for_whom( $group );
}
	function bp_get_group_for_whom( $group = false ) {
		global $groups_template;

		if ( empty( $group ) )
			$group =& $groups_template->group;

		return apply_filters( 'bp_get_group_for_whom', $group->for_whom );
	}
function bp_group_master_fio( $group = false ) {
	echo bp_get_group_master_fio( $group );
}
	function bp_get_group_master_fio( $group = false ) {
		global $groups_template;

		if ( empty( $group ) )
			$group =& $groups_template->group;

		return apply_filters( 'bp_get_group_master_fio', $group->master_fio );
	}
function bp_group_master_telephone( $group = false ) {
	echo bp_get_group_master_telephone( $group );
}
	function bp_get_group_master_telephone( $group = false ) {
		global $groups_template;

		if ( empty( $group ) )
			$group =& $groups_template->group;

		return apply_filters( 'bp_get_group_master_telephone', $group->master_telephone );
	}
function bp_group_master_email( $group = false ) {
	echo bp_get_group_master_email( $group );
}
	function bp_get_group_master_email( $group = false ) {
		global $groups_template;

		if ( empty( $group ) )
			$group =& $groups_template->group;

		return apply_filters( 'bp_get_group_master_email', $group->master_email );
	}
...
function bp_group_admin_tabs( $group = false ) {
	...
	if ( bp_is_item_admin() ) : ?>
		...
		<div<?php if ( 'edit-details' == $current_tab || empty( $current_tab ) )  { ?> class="current btn btn-default"<?php } else { ?> class=" btn btn-default"<?php } ?>><a href="<?php echo trailingslashit( bp_get_group_permalink( $group ) . 'admin/edit-details' ) ?>"><?php _e( 'Details', 'buddypress' ); ?></a></div>
	...
	<div<?php if ( 'group-settings' == $current_tab ) { ?> class="current btn btn-default"<?php } else { ?> class=" btn btn-default"<?php } ?>><a href="<?php echo trailingslashit( bp_get_group_permalink( $group ) . 'admin/group-settings' ) ?>"><?php _e( 'Settings', 'buddypress' ); ?></a></div>

	<?php if ( !(int)bp_get_option( 'bp-disable-avatar-uploads' ) ) : ?>

		<div<?php if ( 'group-avatar'   == $current_tab )  { ?> class="current btn btn-default"<?php } else { ?> class=" btn btn-default"<?php } ?>><a href="<?php echo trailingslashit( bp_get_group_permalink( $group ) . 'admin/group-avatar' ) ?>"><?php _e( 'Avatar', 'buddypress' ); ?></a></div>

	<?php endif; ?>

	<div<?php if ( 'manage-members' == $current_tab )  { ?> class="current btn btn-default"<?php } else { ?> class=" btn btn-default"<?php } ?>><a href="<?php echo trailingslashit( bp_get_group_permalink( $group ) . 'admin/manage-members' ) ?>"><?php _e( 'Members', 'buddypress' ); ?></a></div>

	<?php if ( $groups_template->group->status == 'private' ) : ?>

		<div<?php if ( 'membership-requests' == $current_tab )  { ?> class="current btn btn-default"<?php } else { ?> class=" btn btn-default"<?php } ?>><a href="<?php echo trailingslashit( bp_get_group_permalink( $group ) . 'admin/membership-requests' ) ?>"><?php _e( 'Requests', 'buddypress' ); ?></a></div>

	<?php endif; ?>

	<?php do_action( 'groups_admin_tabs', $current_tab, $group->slug ) ?>

	<div<?php if ( 'delete-group' == $current_tab )  { ?> class="current btn btn-default"<?php } else { ?> class=" btn btn-default"<?php } ?>><a href="<?php echo trailingslashit( bp_get_group_permalink( $group ) . 'admin/delete-group' ) ?>"><?php _e( 'Delete', 'buddypress' ); ?></a></div>
...
function bp_group_member_avatar_mini( $width = 120, $height = 120 ) {
	echo bp_get_group_member_avatar_mini( $width, $height );
}
	function bp_get_group_member_avatar_mini( $width = 120, $height = 120 ) {
		global $members_template;
...
function bp_new_group_for_whom() {
	echo bp_get_new_group_for_whom();
}
	function bp_get_new_group_for_whom() {
		global $bp;

		if ( isset( $bp->groups->current_group->for_whom ) )
			$for_whom = $bp->groups->current_group->for_whom;
		else
			$for_whom = '';

		return apply_filters( 'bp_get_new_group_for_whom', $for_whom );
	}
function bp_new_group_place() {
	echo bp_get_new_group_place();
}
	function bp_get_new_group_place() {
		global $bp;

		if ( isset( $bp->groups->current_group->place ) )
			$place = $bp->groups->current_group->place;
		else
			$place = '';

		return apply_filters( 'bp_get_new_group_place', $place );
	}
function bp_new_group_place_coordinates() {
	echo bp_get_new_group_place_coordinates();
}
	function bp_get_new_group_place_coordinates() {
		global $bp;

		if ( isset( $bp->groups->current_group->place_coordinates ) )
			$place_coordinates = $bp->groups->current_group->place_coordinates;
		else
			$place_coordinates = '';

		return apply_filters( 'bp_get_new_group_place_coordinates', $place_coordinates );
	}
function bp_new_group_master_fio() {
	echo bp_get_new_group_master_fio();
}
	function bp_get_new_group_master_fio() {
		global $bp;

		if ( isset( $bp->groups->current_group->master_fio ) )
			$master_fio = $bp->groups->current_group->master_fio;
		else
			$master_fio = '';

		return apply_filters( 'bp_get_new_group_master_fio', $master_fio );
	}
function bp_new_group_master_telephone() {
	echo bp_get_new_group_master_telephone();
}
	function bp_get_new_group_master_telephone() {
		global $bp;

		if ( isset( $bp->groups->current_group->master_telephone ) )
			$master_telephone = $bp->groups->current_group->master_telephone;
		else
			$master_telephone = '';

		return apply_filters( 'bp_get_new_group_master_telephone', $master_telephone );
	}
function bp_new_group_master_email() {
	echo bp_get_new_group_master_email();
}
	function bp_get_new_group_master_email() {
		global $bp;

		if ( isset( $bp->groups->current_group->master_email ) )
			$master_email = $bp->groups->current_group->master_email;
		else
			$master_email = '';

		return apply_filters( 'bp_get_new_group_master_email', $master_email );
	}
...
function bp_directory_groups_search_form() {
	...
	$search_form_html = '<form action="" method="get" id="search-groups-form">
		<label><input type="text" name="s" id="groups_search" placeholder="Поиск по семинарам" /></label>
