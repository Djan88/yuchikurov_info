...
class BP_Groups_List_Table extends WP_List_Table {
	...
	*/
	...
	function column_description( $item = array() ) {
                $item['description'] = mb_substr($item['description'], 0, 200, 'UTF-8') . '...';
	...
	function column_last_active( $item = array() ) {
        ...
        /*
         * 
         * Добавляю пользовательские методы
         * 
         */
        
        function column_sity( $item = array() )
        {
            return $city = groups_get_groupmeta( $item['id'], 'group_plus_header_fieldthree');
        }

        function column_master( $item = array() )
        {
            $group = groups_get_group( array( 'group_id' => $item['id'] ) );
            return $master_name = bp_core_get_user_displayname( $group->admins[0]->user_id );   
        }
        
        function column_assistant( $item = array() )
        {
            $group = groups_get_group( array( 'group_id' => $item['id'] ) );
            
            if(!empty($group->mods[0]->user_id)) {  return $assistant = bp_core_get_user_displayname( $group->mods[0]->user_id ); } 
