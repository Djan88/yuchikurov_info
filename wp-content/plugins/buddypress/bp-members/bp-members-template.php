...
function bp_directory_members_search_form() {
	...
	if(is_page(1316)){
		$search_form_html = '<form action="" method="get" id="search-members-form">
			<label><input type="text" name="s" id="members_search" placeholder="Введите имя" /></label>
			<input type="submit" id="members_search_submit" name="members_search_submit" value="Поиск мастера" />
		</form>';
	} else {
		$search_form_html = '<form action="" method="get" id="search-members-form">
		...
		if ( bp_loggedin_user_domain() ) {
			...
			$link = trailingslashit( bp_displayed_user_domain() . $user_nav_item['link'] );
                ...
                if($user_nav_item['css_id'] == 'xprofile'){
                    $user_nav_item['name'] = 'Профиль';
                }
                
                if($user_nav_item['css_id'] == 'settings'){
                    $user_nav_item['name'] = 'Настройки';
                }
                
                if($user_nav_item['css_id'] == 'groups' && !xprofile_get_field_data( 23, bp_displayed_user_id())){
                    
                } else {
                    
                    if($user_nav_item['css_id'] == 'reception_days'){
                        
                        if(xprofile_get_field_data( 18, bp_displayed_user_id())){
                    ...
                    } elseif($user_nav_item['css_id'] == 'past_seminars') {
                        
                        if(bp_is_my_profile() && pc_get_userrole(bp_displayed_user_id()) == 'author'){
                            echo apply_filters_ref_array( 'bp_get_displayed_user_nav_' . $user_nav_item['css_id'], array( '<li id="' . $user_nav_item['css_id'] . '-personal-li" ' . $selected . '><a id="user-' . $user_nav_item['css_id'] . '" href="' . $link . '">' . $user_nav_item['name'] . '</a></li>', &$user_nav_item ) );
                        }
                        
                    } elseif($user_nav_item['css_id'] == 'author_news') {
                        
                        if(pc_get_userrole(bp_displayed_user_id()) == 'author' || bp_displayed_user_id() == 1){
                            echo apply_filters_ref_array( 'bp_get_displayed_user_nav_' . $user_nav_item['css_id'], array( '<li id="' . $user_nav_item['css_id'] . '-personal-li" ' . $selected . '><a id="user-' . $user_nav_item['css_id'] . '" href="' . $link . '">' . $user_nav_item['name'] . '</a></li>', &$user_nav_item ) );
                        }
                        
                    } elseif($user_nav_item['css_id'] == 'author_books') {
                        
                        if(bp_displayed_user_id() == 1){
                            echo apply_filters_ref_array( 'bp_get_displayed_user_nav_' . $user_nav_item['css_id'], array( '<li id="' . $user_nav_item['css_id'] . '-personal-li" ' . $selected . '><a id="user-' . $user_nav_item['css_id'] . '" href="' . $link . '">' . $user_nav_item['name'] . '</a></li>', &$user_nav_item ) );
                        }
                        
                    } elseif($user_nav_item['css_id'] == 'logged_seminar') {
