...
<title><?php do_action('gavernwp_title'); ?></title>
	...
	<?php
	   ...
	   wp_enqueue_style('gavern-extensions', gavern_file_uri('css/extensions.css'), array('gavern-wpextensions'));
           ...
           wp_enqueue_style('mosaic-css', gavern_file_uri('css/mosaic.css'));
	?>
	<!--[if IE 9]>
		...
		<div id="gk-header">
			<div id="gk-top">
				<?php if(gk_show_menu('mainmenu') && get_option($tpl->name . '_menu_type', 'overlay') == 'classic') : ?>
					<?php gavern_menu('mainmenu', 'main-menu-mobile', array('walker' => new GKMenuWalkerMobile(), 'items_wrap' => '<i class="icon-reorder"></i><select onchange="window.location.href=this.value;"><option value="#">'.__('Select a page', GKTPLNAME).'</option>%3$s</select>', 'container' => 'div')); ?>
				<?php endif; ?>
				...
				<div id="gk-user-area">
					...
					<a href="<?php echo get_option($tpl->name . '_login_url', 'wp-login.php?action=login'); ?>" id="gk-login"><?php (!is_user_logged_in()) ? _e('Войти', GKTPLNAME) : _e('Выйти', GKTPLNAME); ?></a>
					<?php endif; ?>
				</div>
				<?php endif; ?>
                                ...
                                <?php if(is_user_logged_in()) { ?>
                                    <div class="header-user-info">
                                        <?php 
                                            global $userdata; 
                                            get_currentuserinfo();
                                            echo '<a href="/members/'.$userdata->user_login.'">';
                                ...
                                <?php } else { ?>
                                    ...
                                    <div class="header-user-info">
                                        <a href="/личная-сраница/" title="Создать личную страницу"><i class="icon-plus"></i></a>
                                    </div>
                            
                                <?php } ?>
                                
                                <div class="faq-menu">
                                    <a href="http://wizardmachine.ru/forum/">КЛУБ</a>
                                </div>
