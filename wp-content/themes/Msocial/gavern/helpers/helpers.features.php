...
/**
 *
 * Code to load buddypress css rules
 *
 **/
if(get_option($tpl->name . "_buddypress_css", 'Y') == 'Y') {
	function bp_dtheme_enqueue_styles() {
	     // gk buddypress css
	     wp_enqueue_style('gavern-buddypress', gavern_file_uri('css/gk.buddypress.css'), array('gavern-extensions'));
	}
			...
			<fieldset class="gk_widget_rules_form" id="gk_widget_rules_form_'.$unique_id.'" data-id="gk_widget_rules_form_'.$id.'">
				<legend>'.__('Select page to add', GKTPLNAME).'</legend>
				 <select class="gk_widget_rules_form_select">
				 	...
				 	<option value="buddypress">'.__('BuddyPress Activity', GKTPLNAME).'</option>
...
/**
 * Provides login form for use anywhere within WordPress. By default, it echoes
 ...
 */
function gk_login_form( $args = array() ) {
	$defaults = array( 'echo' => true,
						'redirect' => ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], // Default redirect is back to the current page
	 					'form_id' => 'loginform',
						'label_username' => __( 'Username' ),
						'label_password' => __( 'Password' ),
						'username_placeholder' => __( 'Username' ),
						'password_placeholder' => __( 'Password' ),
	...
	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'wp-login.php', 'login_post' ) ) . '" method="post">
			...
			<p class="login-username">
				<input type="text" name="log" id="' . esc_attr( $args['id_username'] ) . '" class="input" value="' . esc_attr( $args['value_username'] ) . '" size="20" placeholder="' .esc_attr( $args['username_placeholder'] ) . '" />
			</p>
			<p class="login-password">
				<input type="password" name="pwd" id="' . esc_attr( $args['id_password'] ) . '" class="input" value="" size="20" placeholder="' .esc_attr( $args['password_placeholder'] ) . '" />
			</p>
