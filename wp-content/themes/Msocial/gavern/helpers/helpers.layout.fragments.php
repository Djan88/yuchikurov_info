...
function gavern_menu($menuname, $fullname, $params = null) {			
	...
	if(gk_show_menu($menuname)) {
		...
		wp_nav_menu(array(
			  ...
			  'menu_class'      => 'menu ' . $tpl->menu[$menuname]['style'] . ' ' . ((get_option($tpl->name . '_menu_type', 'overlay') == 'overlay') ? 'gk-menu-overlay' : 'gk-menu-classic'), 
...
function gavern_comment_template( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>">	
			<aside>
				<?php echo get_avatar( $comment, ($comment->comment_parent == '0') ? 60 : 52); ?>
			</aside>
...
function gk_post_meta($attachment = false) {
    ...
    if($params_aside) {
 	...
 	?>
 		<?php if($param_aside) : ?>
	 	<ul>	 		 		
	 		<?php if((is_tag() || is_search()) || (get_post_type() != 'page' && $param_category)) : ?>
		 	<li class="category">
		 		<?php _e('Опубликовано:', GKTPLNAME); ?>
...
function gk_content_nav($id = '') {
	...
	if($wp_query->max_num_pages > 1) : ?>
		...
		<nav class="pagenav"<?php if($id != '') : ?> id="<?php echo $id; ?>"<?php endif; ?>>
			...
			<div class="nav-prev nav-btn"><?php next_posts_link( __( 'Предыдущие записи', GKTPLNAME ) ); ?></div>
			<?php endif; ?>
			
			<?php if(get_previous_posts_link() != '') : ?>
			<div class="nav-next nav-btn"><?php previous_posts_link( __( 'Новые записи', GKTPLNAME ) ); ?></div>
			<?php endif; ?>
		</nav><!-- #nav-above -->
			        ...
			        <div class="author-desc">
			            <h2>
			                <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
			                    <?php printf( __( '%s ', GKTPLNAME ), get_the_author_meta('display_name', get_the_author_meta( 'ID' )) ); ?> 
			                </a>
			            </h2>
			            <p>
