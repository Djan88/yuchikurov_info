...
function gk_blog_logo() {
	...
	if(get_option($tpl->name . "_branding_logo_type", 'css') == 'image') {
		...
		if(get_option($tpl->name . "_branding_logo_text_type", 'wp') == 'wp') {
			...
			$logo_text = preg_replace('/__(.*?)__/i', '<span>${1}</span>', get_option($tpl->name . "_branding_logo_text_value", ''));
...
function gk_opengraph_metatags() {
	...
	if(get_option($tpl->name . '_opengraph_use_opengraph') == 'N') {
		if(is_single() || is_page()) {
			...
			//
			echo apply_filters('gavern_og_title', '<meta name="og:title" content="'.$title.'" />' . "\n");
			//
			if($image != '') {
...
function gk_breadcrumbs_output() {
	...
	$output = '<nav class="gk-breadcrumbs gk-page">';
	...
	if($mode != 'all') {
		...
		for($i = 0; $i < count($input); $i++) {
			...
			if(stripos($input[$i], 'homepage') !== FALSE) {
			    $output .= ' is_home() ';
		    } else if(stripos($input[$i], 'buddypress') !== FALSE) {
		        $output .= ' bp_is_activity_component() ';
...
function gk_is_active_sidebar( $index ) {
	...
	return $widget_counter == 0 ? false : $widget_counter;
}
