...
?>		
	...
	<?php if(get_option($tpl->name . '_template_homepage_mainbody', 'N') == 'N') : ?>
		...
		<?php if ( have_posts() ) : ?>		
			<section id="gk-mainbody">
				...
				<?php
					$frontpage_amount = (int) get_option($tpl->name . '_template_homepage_mainbody_amount', '1');
					$iterator = 0;
				?>
				...
				<?php
					if(get_option($tpl->name . '_template_homepage_mainbody_nav', 'Y') == 'Y') { 
				 		gk_content_nav(); 
				 	}
				?>
	...
	<?php else: ?>
		...
		<section id="gk-mainbody" class="contains-widget">
